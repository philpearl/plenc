...
func (c JSONMapCodec) Read(data []byte, ptr unsafe.Pointer, wt WireType) (n int, err error) {
	...
	for ; count > 0; count-- {
		...
		n, err := c.readKV(data[offset:offset+int(l)], &key, &val)
		if err != nil {
			return 0, err
		}
		offset += n
...
func (c JSONMapCodec) readKV(data []byte, key *string, val *interface{}) (n int, err error) {
	var (
		jType  jsonType
		offset int
	)
...
func (c JSONArrayCodec) Size(ptr unsafe.Pointer) (size int) {
	...
	for _, val := range a {
		itemSize := c.sizeV(val)
		size += SizeVarUint(uint64(itemSize)) + itemSize
	}
	return size
}

func (c JSONArrayCodec) sizeV(v interface{}) (size int) {
	size += SizeTag(WTVarInt, 2)
	switch v := v.(type) {
	case nil:
		size += SizeVarUint(uint64(jsonTypeNil))
...
func (c JSONArrayCodec) Append(data []byte, ptr unsafe.Pointer) []byte {
	...
	for _, val := range a {
		...
		data = c.appendV(data, val)
	}
	return data
}

func (c JSONArrayCodec) appendV(data []byte, v interface{}) []byte {
	data = AppendTag(data, WTVarInt, 2)
	switch v := v.(type) {
	case nil:
		data = AppendVarUint(data, uint64(jsonTypeNil))
...
func (c JSONArrayCodec) Read(data []byte, ptr unsafe.Pointer, wt WireType) (n int, err error) {
	...
	for i := range a {
		...
		n, err := c.readV(data[offset:offset+int(l)], &a[i])
		if err != nil {
			return 0, err
		}
		offset += n
...
func (c JSONArrayCodec) readV(data []byte, val *interface{}) (n int, err error) {
	var (
		jType  jsonType
		offset int
	)
	...
	for offset < len(data) {
		...
		case 2:
			v, n := ReadVarUint(data[offset:])
			if n < 0 {
				return 0, fmt.Errorf("invalid array type field")
			}
			jType = jsonType(v)
			offset += n
		case 3:
